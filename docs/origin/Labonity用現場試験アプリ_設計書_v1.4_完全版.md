# Labonity 用現場試験アプリ 設計書 v1.4 完全版

**出荷実績写真保存・ローカル写真自動保存・写真保存フォルダ指定・出荷別JPEGエクスポート・OCR取込用黒板撮影・事前OCR・Sync Agent サーバー配置・LibLocal.xml 起点DB接続・Sync Agent 非人間主体認証・Agent credential・Google Maps 連携・Liberty Account 認証・TP採取結果入力へのOCR結果反映方式・NaDat ローカル連携テーブル・OCR反映先安全検証・Sync Agent 監視強化・端末ライブラリOCR黒板登録**
...
本版は v1.3 の内容を削除せず、レビューで残っていた運用・実装上の不確定点を解消した完全版である。特に、出荷実績未同期時の扱い、ローカル連携テーブル配置、ID 型、OCR 反映先検証、Sync Agent の状態管理・監視、ローカル JPEG 生成監視、Agent credential の漏えい時対応、パス長制御、OCR 業務妥当性検証を確定した。

### 0.0.1 v1.4 の追加設計判断

| 論点 | v1.4 の判断 |
|---|---|
| 出荷実績未同期時 | アプリ内で未紐づけ写真は作らない。現場担当者はスマホ標準カメラで写真を撮影しておき、出荷実績が同期された後に端末ライブラリから対象出荷へ登録する。 |
| ライブラリ写真の OCR 利用 | 端末ライブラリから選択した写真も、専用導線で `capture_purpose = fresh_test_ocr_blackboard` として登録できる。通常写真として登録した写真は OCR 対象にしない。 |
| ローカル連携テーブル配置 | `FieldPhotoReference`、`FieldPhotoLocalFile`、`FieldPhotoMaterializeJob`、`FieldPhotoExportConfig`、`FieldOcrResultReference`、`FieldOcrResultFieldReference`、`FieldOcrImportAudit`、Sync Agent 管理テーブルは、出荷管理 DB である `NaDat.dbo` に作成する。 |
| TP データとの関係 | TP 採取結果本体、フレッシュ試験、TP と出荷の関係は既存どおり `ExDat` を正とする。Labonity 画面は `ExDat` の現在コンテキストを確定し、OCR・写真参照は `NaDat` を検索する。 |
| ID 型 | `tenant_id` は Liberty Account の `orgId` を保存する `nvarchar(64)`、`plant_id` は Labonity の `kozyo_id` を保存する `uniqueidentifier`、`yotei_id`、`syukka_id`、`genba_id`、`testpiecesaisyu_main_id` は既存 Labonity と同じ `uniqueidentifier` とする。API ではすべて canonical string として送受信する。 |
| OCR 反映先安全性 | 保存済み TP では `ExDat.dbo.TestPieceSaisyu_SyukkaData` の `testpiecesaisyu_main_id + renban + syukka_id` 一致を必須検証する。未保存 TP では画面メモリ上の出荷指定コンテキストを検証し、保存時に通常の Labonity 保存で関係を確定する。 |
| 反映画面の誤操作防止 | OCR 結果確認画面に、反映先 TP、`renban`、`datakubun`、出荷日時、車番、出荷数量、現場名を表示する。検証不一致時は反映ボタンを無効化する。 |
| 起動時 fail-fast と運用中リトライ | 設定不足・権限不足・必要テーブル不足は起動時構成エラーとして同期開始禁止。運用中の一時的な DB / 共有フォルダ / クラウド障害は `degraded_retrying` としてサービスを生かしたまま再試行する。 |
| Sync Agent 単一障害対策 | 同一 `tenant_id + plant_id` では active agent は 1 つのまま、heartbeat、lease、最終同期時刻、未処理イベント数、未生成写真数、失敗ジョブ数、最古滞留時刻を可視化し、閾値超過を管理者確認対象にする。 |
| JPEG 未生成のサイレント化防止 | PhotoReferenceEvent の ACK と JPEG 生成は分離するが、MaterializeJob の滞留・失敗を `NaDat` と heartbeat に記録し、Labonity と管理診断で確認できるようにする。 |
| Agent credential の追加制御 | access token の audience を sync API 専用にし、download-url は `orgId + plantId + photoAssetId` と対象イベントの整合を API 側で検証する。credential 漏えい時の失効・再発行・ローテーション手順を初期運用に含める。 |
| パス長制御 | パス長超過を検知するだけでなく、生成時に `MaxFullPathLength` 内へ収める。初期値は legacy Windows 互換を考慮して 240 文字以内を推奨する。 |
| OCR 値の業務妥当性 | 型変換、DB 桁数、車番一致、項目別範囲、単位、丸め、`datakubun`、既存値差分を検証し、異常値は要確認または反映不可にする。 |
| OCR 修正値の二次利用 | 初期リリースではモデル改善用の外部二次利用は無効にする。監査・障害調査用には保持し、モデル改善に使う場合は別途管理設定と運用承認を必要とする。 |
| ローカル写真の個人情報・保持 | 初期リリースでは既存方針を維持し、追加の個人情報対策は本版の必須解決対象外とする。 |

### 0.0.2 v1.4 の最終判断

```text
ローカル連携テーブルは NaDat.dbo に作成する。
TP 採取結果入力の正式データは ExDat を正とする。
Labonity 側は ExDat の画面コンテキストと NaDat の OCR・写真参照を組み合わせて表示する。
出荷実績未同期時、アプリ内で写真先行登録は行わない。
現場担当者はスマホ標準カメラで写真を撮影し、出荷実績同期後に端末ライブラリから通常写真または OCR取込用黒板写真として登録する。
端末ライブラリから選択した写真も、専用操作なら OCR取込用黒板写真として登録できる。
OCR 反映時は syukka_id だけでなく、画面の testpiecesaisyu_main_id + renban + datakubun と出荷指定コンテキストを検証する。
保存済み TP では ExDat.dbo.TestPieceSaisyu_SyukkaData の testpiecesaisyu_main_id + renban + syukka_id 一致を必須にする。
起動時構成不備は fail-fast、運用中一時障害は degraded_retrying として再試行する。
Sync Agent は heartbeat と MaterializeJob 監視により、同期停止、写真未生成、OCR未同期をサイレント化しない。
Agent credential は sync API 専用 audience と最小 scope に制限し、漏えい時は失効・再発行・ローテーションできる。
ファイルパスは生成時に上限内へ短縮し、Labonity から開けない長さを作らない。
OCR 値は信頼度だけでなく、業務妥当性、型、桁数、既存値差分を検証してから反映候補にする。
```

---

## 0.1 v1.3 改訂サマリー（本版でも保持）

本版は v1.2 の内容を削除せず、**認証基盤との接続方針を明確化し、現場試験 Web アプリの個人認証と Sync Agent の非人間主体認証を分離する方式**を追加した改訂版である。
...
### 0.0.2 v1.3 で追加する主な要素

- Sync Agent の **非人間主体認証** と Agent credential の定義。
- Sync Agent に個人アカウント、個人パスワード、個人 refresh token を保持させないルール。
- Agent credential による短命 access token 取得方式。
- `orgId + plantId + agentId` による認可スコープ固定。
- `/api/sync/v1/...` 専用のスコープ設計。
- TenantAdmin による Agent 登録・失効・再発行・ローテーション運用。
- 初回登録コードの短時間・一回限り運用。
- Agent credential の安全な保存、ログマスク、診断表示ルール。
- 監査ログの `actor_type = user / agent` 分離。
- 受入条件 A-81 以降に、Sync Agent 非人間主体認証の条件を追加。

### 0.0.3 v1.3 の最終判断

```text
現場試験 Web アプリは Liberty Account の個人認証を使用する。
...
## 0.2 v1.2 改訂サマリー（本版でも保持）

本版は v1.1 の内容を削除せず、**Labonity クライアントサーバー構成における Sync Agent のサーバー配置、Labonity の `Settings\LibLocal.xml` を起点にした DB 接続自動解決、写真保存フォルダの明示的なパス指定方式**を追加した改訂版である。

### 0.0.1 v1.2 の追加設計判断

| 論点 | v1.2 の判断 |
|---|---|
| Sync Agent の配置 | 本番運用では **Labonity サーバーへインストールすることを原則必須**とする。クライアント PC ごとにはインストールしない。 |
...
### 0.0.2 v1.2 で追加する主な要素

- Sync Agent の本番配置先を **Labonity サーバー** に固定する運用設計。
- Sync Agent の設定項目 `LabonityLocalSettingFilePath`。
- `LibLocal.xml` から `p_サーバーフォルダパス` を読み、`LibertyDatabaseSetting.xml` を自動検出する DB 接続解決方式。
- DB 接続情報の二重入力禁止と、設定不足時の fail-fast ルール。
- 写真保存先の `WriterRootPath` / `LabonityVisibleRootPath` 分離。
- 複数クライアント運用時の UNC 共有パス標準化。
- Sync Agent 二重起動防止、サービスアカウント権限、起動時ヘルスチェック。
- 受入条件 A-67 以降に、サーバー配置、LibLocal.xml 起点 DB 接続、写真フォルダ指定の条件を追加。

### 0.0.3 v1.2 の最終判断

```text
Sync Agent は本番では Labonity サーバーにインストールする。
...
### 0.0.4 v1.1 で確定済みの改訂内容（本版でも保持）


本版は v1.0 の内容を削除せず、**現場アプリで撮影された写真を Sync Agent がローカルの指定フォルダへ自動保存する方式**を追加した改訂版である。
...
| Sync Agent の役割 | クラウドとの同期だけでなく、写真ファイルをローカルへ**マテリアライズ**する。通常写真、OCR取込用黒板写真、サムネイル、メタデータを対象にする。 |
...
#### 0.0.4.2 v1.1 で追加した主な要素

- `FieldPhotoLocalFile` によるローカル写真ファイル管理。
- `FieldPhotoMaterializeJob` による写真ダウンロード・JPEG 正規化・出荷別フォルダ出力の再試行管理。
- `FieldPhotoExportConfig` による出力先ルート、フォルダ命名規則、保持期間、JPEG 品質、通常写真の出力有無の設定。
- Sync Agent の「内部キャッシュ」と「業務用エクスポートフォルダ」の二層構成。
- Labonity 側の参照ルールを「ローカルDBのパスを取得して、通常ファイルとして開く」に固定。
- 受入条件 A-54 以降に、ローカル写真自動保存と API/Named Pipe 非使用の条件を追加。
...
### 0.4 v1.4 固定 ID・DB 配置ルール

本版では、ID 型とローカル連携テーブルの配置を次の通り固定する。以後の表や SQL 例で旧版の `uuid / string`、`uniqueidentifier / varchar` のような表記が残る場合も、本節を正とする。
...
| `NaDat.dbo` | 追加 | 現場アプリ連携用の写真参照、OCR参照、MaterializeJob、監査、Sync Agent 管理テーブルを配置する。 |
...
3. NaDat.dbo.FieldOcrResultReference / FieldOcrResultFieldReference を syukka_id で検索する。
4. NaDat.dbo.FieldPhotoReference / FieldPhotoLocalFile を syukka_id で検索する。
5. ユーザー確認後、値は ExDat の FreshSiken 画面入力欄へ反映する。
6. DB 保存は既存 Labonity の通常保存で行う。
```

同一 SQL Server 上で cross database query が利用できる場合は、`NaDat.dbo` と `ExDat.dbo` を完全修飾して参照してよい。権限・構成上 cross database query を避ける場合は、アプリ側で `ExDat` 参照と `NaDat` 参照を 2 段階に分ける。
...
sequenceDiagram
    ...
    participant Cloud as Labonity Cloud
...
### 3.7.5 v1.4 Agent credential 追加制御
...
### 4.8 v1.4 ローカル連携テーブル配置と Sync Agent 管理テーブル

本機能で追加するローカル連携テーブルは、すべて `NaDat.dbo` に作成する。
...
出荷予定詳細画面で対象の出荷実績がまだ表示されていない場合、写真追加ボタン、OCR取込用黒板撮影ボタン、OCR取込用黒板ライブラリ選択ボタンは表示しない、または非活性にする。
...
| 出荷実績一覧に対象出荷が表示された | 通常の出荷実績詳細から、カメラ撮影、通常写真ライブラリ選択、OCR黒板撮影、OCR黒板ライブラリ選択を許可する。 |
...
| OCR取込用黒板を撮影 | `camera` | `fresh_test_ocr_blackboard` | 対象 |
...
テーブル名: `NaDat.dbo.FieldPhotoReference`
...
```sql
SELECT TOP (1) 1
FROM NaDat.dbo.FieldPhotoReference
...
### 7.7 v1.1 ローカル写真自動保存の位置づけ

7.6 のローカル写真キャッシュは、OCR結果確認画面で読取元画像を表示するための内部キャッシュである。v1.1 ではこれに加えて、Labonity デスクトップアプリや写真確認業務で直接扱うための **ローカル写真エクスポート**を追加する。
...
テーブル名: `NaDat.dbo.FieldPhotoLocalFile`
...
テーブル名: `NaDat.dbo.FieldPhotoMaterializeJob`
...
テーブル名: `NaDat.dbo.FieldPhotoExportConfig`
...
SELECT
    ...
    f.size_bytes,
    f.sha256_hash
FROM NaDat.dbo.FieldPhotoReference r
LEFT JOIN NaDat.dbo.FieldPhotoLocalFile f
...
出荷実績が指定された後、Labonity 側は `NaDat.dbo.FieldOcrResultReference` を参照し、対象 `syukka_id` の OCR 結果を確認する。
...
```sql
SELECT TOP (1) *
FROM NaDat.dbo.FieldOcrResultReference
WHERE tenant_id = @tenant_id
  AND plant_id = @plant_id
  AND syukka_id = @syukka_id
  AND ocr_usage = 'fresh_test_blackboard'
...
```sql
SELECT *
FROM NaDat.dbo.FieldOcrResultFieldReference
...
```sql
SELECT TOP (1) *
FROM NaDat.dbo.FieldOcrImportAudit
WHERE tenant_id = @tenant_id
  AND plant_id = @plant_id
...
テーブル名: `NaDat.dbo.FieldOcrResultReference`
...
テーブル名: `NaDat.dbo.FieldOcrResultFieldReference`
...
Labonity 側で OCR 反映結果を追跡するため、独立した監査テーブル `NaDat.dbo.FieldOcrImportAudit` を使用する。
...
- 「OCR取込用黒板をライブラリから選択」：スマホ標準カメラ等で先に撮影済みの黒板写真を、OCR取込用黒板写真として登録する。
- OCR取込用黒板写真は通常写真の分類ではなく、保存後に OCR ジョブ予約へ進む専用操作として扱う。カメラ撮影でもライブラリ選択でも、専用操作を通った場合だけ `capture_purpose = fresh_test_ocr_blackboard` とする。
- 「キャンセル」ボタンでメニューを閉じる。
...
| 用途 | 初期リリース扱い |
|---|---|
| 業務監査 | 有効。`NaDat.dbo.FieldOcrImportAudit` に記録する。 |
...
| 運用中のクラウド API 一時失敗 | `degraded_retrying` としてリトライし、失敗回数・最終エラーを `NaDat.dbo.FieldSyncLog` に記録する。 |
...
| A-95 | ローカル連携テーブルは `NaDat.dbo` に作成される。 |
...
| A-112 | `NaDat` に追加するローカル連携テーブルの作成・更新スクリプトを用意し、起動時に存在と権限を検証できる。 |
| A-113 | Labonity 側は `ExDat` の TP コンテキストと `NaDat` の OCR・写真参照を、cross database query またはアプリ側 2 段階参照で取得できる。 |
...
- Sync Agent は `NaDat.dbo` にローカル連携テーブルと管理テーブルが存在すること、参照・更新権限があることを起動時に検証する。
...
- `NaDat.dbo.FieldPhotoLocalFile` / `NaDat.dbo.FieldPhotoMaterializeJob` / `NaDat.dbo.FieldPhotoExportConfig` を実装する。
- `NaDat.dbo.FieldSyncCheckpoint` / `NaDat.dbo.FieldSyncLog` / `NaDat.dbo.FieldSyncAgentHealth` / `NaDat.dbo.FieldSyncAgentLease` を実装する。
...
- 出荷実績詳細画面または写真追加メニューに [OCR取込用黒板をライブラリから選択] を配置する。
...
## 19. v1.1 ローカル写真自動保存・出荷別JPEGエクスポート設計
...
## 20. v1.2 Sync Agent サーバー配置・LibLocal.xml 起点DB接続・写真保存パス指定設計
...
| `p_品質管理データベース名` | TP採取結果、OCR参照、写真参照テーブルの主な配置先。通常 `EXDAT`。 |
...
## 21. v1.3 Sync Agent 非人間主体認証・Agent credential 設計
...
| No | 事項 | 解決内容 |
|---|---|---|
| R-01 | 出荷実績未同期時に写真登録できない問題 | アプリ内未紐づけ保存は行わず、スマホ標準カメラで撮影後、出荷実績同期後にライブラリから登録する運用に確定。 |
| R-02 | OCR黒板写真を後から選べない懸念 | ライブラリ写真を OCR取込用黒板写真として登録する専用導線を必須化。 |
| R-03 | `syukka_id` だけで OCR 反映する誤操作リスク | `testpiecesaisyu_main_id + renban + syukka_id` の一致検証、未保存 TP の画面コンテキスト検証、反映先表示、重複反映検知を追加。 |
| R-04 | 起動時 fail-fast と運用中リトライの矛盾 | 起動時構成エラーは同期開始禁止、運用中一時障害は `degraded_retrying` で再試行に整理。 |
| R-05 | Sync Agent 単一障害点 | active agent は 1 つのまま、heartbeat、lease、管理診断、手動フェイルオーバーを明記。 |
| R-06 | JPEG 未生成のサイレント化 | MaterializeJob、FieldPhotoLocalFile、FieldSyncAgentHealth、heartbeat、Labonity 表示で滞留・失敗を可視化。 |
| R-07 | Agent credential 漏えい時の影響 | audience、scope、download-url 制限、失効・再発行・ローテーション手順を追加。 |
| R-08 | ID 型の揺れ | `tenant_id = nvarchar(64)`、`plant_id = kozyo_id uniqueidentifier`、source local ID は Labonity の uniqueidentifier に固定。 |
| R-09 | ローカル連携テーブルの配置 | すべて `NaDat.dbo` に配置することで確定。 |
| R-10 | パス長問題 | `MaxFullPathLength` と短縮アルゴリズムを追加し、生成時に上限内へ収める。 |
| R-11 | OCR 値の妥当性不足 | DB 型、桁数、業務範囲、単位、車番一致、既存値差分の検証を追加。 |
| R-12 | OCR 修正値の二次利用 | 初期リリースではモデル改善用二次利用を無効化し、監査・障害調査用途に限定。 |
| R-13 | ローカル写真の個人情報・保持 | 今回は既存方針を維持し、追加対策は本版の必須解決対象外。 |

### 22.2 v1.4 実装時の最小必須差分

```text
1. NaDat.dbo にローカル連携テーブルと Sync Agent 管理テーブルを追加する。
2. 現場アプリに「OCR取込用黒板をライブラリから選択」を追加する。
3. 出荷実績未同期画面に、標準カメラ撮影後にライブラリ登録する案内を追加する。
4. Labonity OCR 反映前に ExDat.dbo.TestPieceSaisyu_SyukkaData の一致検証を追加する。
5. OCR結果確認画面に反映先 TP / renban / datakubun / 出荷情報を表示する。
6. Sync Agent の起動時ヘルスチェック、FieldSyncAgentHealth、heartbeat を実装する。
7. MaterializeJob の滞留・失敗を管理診断と Labonity 表示で確認できるようにする。
8. Agent credential の audience / scope / download-url 制限、失効・ローテーション手順を実装する。
9. ローカル写真パス生成時に MaxFullPathLength を適用する。
10. OCR 値の DB 型・桁数・業務範囲検証を実装する。
```

### 22.3 v1.4 最終ルール

```text
ローカル連携テーブルは NaDat.dbo に置く。
TP 採取結果本体は ExDat を正とする。
出荷実績未同期時は、アプリ内未紐づけ写真を作らない。
必要な写真はスマホ標準カメラで撮影し、出荷実績同期後にライブラリから登録する。
ライブラリ写真は、通常写真または OCR取込用黒板写真として明示的に登録できる。
OCR 対象は capture_purpose = fresh_test_ocr_blackboard の写真だけである。
OCR 反映時は、syukka_id だけでなく、画面の TP コンテキストと出荷指定の一致を検証する。
保存済み TP では ExDat.dbo.TestPieceSaisyu_SyukkaData の一致を必須にする。
起動時構成不備は fail-fast、運用中一時障害は degraded_retrying とする。
Sync Agent の停止・滞留・写真未生成は heartbeat と NaDat の管理テーブルで検知する。
Agent credential は個人 token ではなく、sync API 専用の非人間主体 credential として扱う。
ローカル JPEG パスは生成時に短縮し、Labonity で開ける長さに収める。
OCR 値は信頼度、DB 型、桁数、業務範囲、既存値差分を確認してから反映候補にする。
```

---

## 23. v1.4 NaDat 追加テーブル DDL 方針

### 23.1 配置

本機能で追加するローカル連携テーブルは、すべて `NaDat.dbo` に作成する。既存の `NaDat` 出荷管理テーブル、`MsDat` 現場マスター、`ExDat` TP採取結果入力テーブルは変更しない。
...
| バックアップ | `NaDat` の通常バックアップに含める。 |

### 23.3 参考 SQL

DDL 参考スクリプトは別ファイル `NaDat_FieldTest_LocalTables_v1.4.sql` として出力する。設計上のテーブル定義は本文の各テーブルレイアウトを正とし、実装時は実環境の SQL Server バージョン、照合順序、既存命名規則に合わせて調整する。

対象テーブルは次の通りである。

- `dbo.FieldPhotoReference`
- `dbo.FieldPhotoLocalFile`
- `dbo.FieldPhotoMaterializeJob`
- `dbo.FieldPhotoExportConfig`
- `dbo.FieldOcrResultReference`
- `dbo.FieldOcrResultFieldReference`
- `dbo.FieldOcrImportAudit`
- `dbo.FieldOcrValidationRule`
- `dbo.FieldSyncCheckpoint`
- `dbo.FieldSyncLog`
- `dbo.FieldSyncAgentHealth`
- `dbo.FieldSyncAgentLease`
