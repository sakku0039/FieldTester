...
| `photo_asset_target_id` | uniqueidentifier / varchar | クラウド PhotoAssetTarget ID。 |
| `target_type` | nvarchar | 原則 `shipment`。 |
| `target_local_id` | uniqueidentifier / varchar | `SyukkaDataMain.syukka_id`。 |
...
| `photo_asset_id` | uniqueidentifier / varchar | 写真ID。 |
| `target_local_id` | uniqueidentifier / varchar | `syukka_id`。 |
...
| `tenant_id` | uniqueidentifier | テナントID。 |
| `plant_id` | uniqueidentifier / varchar null | 工場別設定。null の場合はテナント共通。 |
...
- FieldPhotoLocalFile / FieldPhotoMaterializeJob / FieldPhotoExportConfig を実装する。
