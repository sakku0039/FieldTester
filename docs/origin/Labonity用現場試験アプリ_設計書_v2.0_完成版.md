# Labonity 用現場試験アプリ 設計書 v2.0 完成版
...
<img src="UI/13_5_写真追加メニュー_OCR対応.png" alt="写真登録メニュー" width="360">
...
DDL 参考スクリプトは別ファイル `NaDat_FieldTest_LocalTables_v2.0.sql` として出力する。設計上のテーブル定義は本文の各テーブルレイアウトを正とし、実装時は実環境の SQL Server バージョン、照合順序、既存命名規則に合わせて調整する。
