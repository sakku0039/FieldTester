# Labonity 用現場試験アプリ 統合設計仕様書 第2.2版
...
DDL 参考スクリプトは別ファイル <a href="09_現場試験アプリ_ローカル連携テーブル定義_第2.2版.sql"><code>09_現場試験アプリ_ローカル連携テーブル定義_第2.2版.sql</code></a> として出力する。設計上のテーブル定義は本文の各テーブルレイアウトを正とし、実装時は実環境の SQL Server バージョン、照合順序、既存命名規則に合わせて調整する。
