...
| 監査 | import、event pull、ack、pending resolve をすべて AuditLog に記録する。 |
...
仮写真は、出荷実績同期後に Sync Agent が正式な `Shipment` へ解決する。
...
| `schedule_source_local_id` | uniqueidentifier / string | ローカル `yotei_id`。 |
| `shipping_date` | date | 出荷日。 |
| `vehicle_no` | nvarchar | 車番。 |
| `shipping_time_hint` | time null | 出荷時刻候補。 |
| `photo_asset_id` | uuid | 写真 ID。 |
| `resolve_status` | nvarchar | `pending` / `resolved` / `ambiguous` / `failed`。 |
| `resolved_shipment_id` | uuid null | 解決後の `Shipment.shipment_id`。 |
| `resolved_syukka_id` | uniqueidentifier / string null | 解決後の `SyukkaDataMain.syukka_id`。 |
| `created_by` | uuid | 登録者。 |
...
+ plant_id
+ shipping_date
+ schedule_source_local_id
+ vehicle_no
+ shipping_time_hint 近傍
```

一致候補が 1 件の場合は `resolved` とし、`PhotoAssetTarget` を `target_type = shipment` で作成する。

候補が複数ある場合は `ambiguous` とし、管理画面または Sync Agent ログで確認対象にする。

### 5.4 Labonity 側候補表示

Labonity 側の写真候補表示は、正式に `Shipment` へ解決された写真を対象とする。仮写真のままの写真は、誤紐づけ防止のため TP採取結果入力画面の OCR 候補には出さない。
...
クラウド側で写真の作成、削除、代表写真変更、表示順変更、対象解決が発生した場合、`PhotoReferenceEvent` を発行する。Sync Agent はイベントを取得し、`FieldPhotoReference` へ反映する。
...
| `event_type` | `created` / `updated` / `deleted` / `primary_changed` / `display_order_changed` / `target_resolved`。 |
| `photo_asset_id` | 写真ID。 |
...
| A-39 | 出荷実績がクラウド未同期の場合、仮写真として登録でき、出荷実績同期後に解決される。 |
