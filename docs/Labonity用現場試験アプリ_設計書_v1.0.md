...
- [5. 画面フローと UI イメージ](#5-画面フローと-ui-イメージ)
  - [5.1 画面遷移図](#51-画面遷移図)
  - [5.2 Google マップ連携仕様（ルート案内）](#52-google-マップ連携仕様ルート案内)
...
- 選択された出荷予定に紐づく現場住所を基に、現在地から現場までの Google マップ（ルート案内）を起動する機能を設ける。
...
| 3 | 現場試験記録 作成/再開 | `FieldTestSession` を作成。既存の未確定・未取込記録があれば再開。 | 現場試験アプリ |
| 4 | 確定・取込状況表示 | 下書き、確定済み、取込完了、競合等のステータスを表示する。 | 現場試験アプリ |
...
### 5.2 Google マップ連携仕様（ルート案内）

現場へ移動する際の補助機能として、選択された出荷予定に紐づく現場の住所情報から、現在地からその現場までの Google マップ（ルート案内）を外部起動する機能を設ける。

- **起動トリガー**: 出荷予定詳細画面などの現場住所表示部分の横に、マップアイコン（または「地図を開く」リンク）を配置する。
- **動作**: アイコン押下時に外部ブラウザまたはマップアプリで Google マップを開く。
- **パラメータ**: 遷移時の URL に目的地（`destination`）として現場住所（または緯度経度）を指定し、現在地からその目的地までのルート検索結果が直接表示されるようにする。
  - 例: `https://www.google.com/maps/dir/?api=1&destination=現場の住所` （`destination` には `FieldSite.zyusyo1` + `FieldSite.zyusyo2`、または `FieldSite.ido` / `FieldSite.keido` をエンコードした値を指定）
...
| 重複確定 | `source_hash` / `idempotency_key` で冪等に処理する。 |

---

## 8. デスクトップ TP アプリ取込設計

デスクトップ TP アプリには、クラウドで確定された `FieldTestSession` / `FieldFreshTestStaging` を確認し、正式な TP 採取データへ反映する「現場記録取込」機能を追加する。
...
- **重複・再確定**：同一フレッシュ試験が誤って複数回確定・取込された場合。
...
- `FieldOperator` は現場記録作成、フレッシュ値入力、写真撮影、黒板保存、現場記録の確定を行える。
...
| Google マップ連携 | 選択した出荷予定の現場情報（住所または緯度経度）から、現在地から現場までの Google マップ（ルート案内）を正しく起動できること。 |
