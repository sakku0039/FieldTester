...
| 忘れ防止 | 出荷実績一覧・詳細に「OCR用黒板 未撮影 / 読取中 / 読取済 / 要確認 / 失敗」を表示する。 |
...
- OCR用黒板状態は `未撮影 / 読取中 / 読取済 / 要確認 / 失敗` を表示する。
...
- 「OCR取込用黒板」カードに撮影状態、OCR状態、読取日時、全体信頼度、要確認件数を表示する。
- 写真サムネイルを横並びで表示する。
...
- 「保存する」、「撮り直す」「写真を選び直す」を配置する。

OCR取込用黒板写真の場合:

- 対象出荷（出荷時刻 / 車番）を上部に表示する。
- 写真プレビューを中央に大きく表示する。
- 「保存後、自動で OCR 読取します。」の説明を表示する。
- 画質チェック（ぼけ、暗さ、傾き、黒板欠け）を表示する。
- 警告がある場合は再撮影を推奨する。
- 「OCR用黒板として保存する」、「撮り直す」を配置する。
